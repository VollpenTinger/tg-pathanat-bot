...
@dataclass
class Preparat:
    id: str           # внутренний id (короткий, латиницей)
    name: str         # полное название по-русски
    category: str     # раздел (для обучения)
...
user_state: Dict[int, Dict] = {}        # временное состояние (режимы, текущие вопросы и т.п.)
user_stats: Dict[int, Dict] = {}        # статистика по пользователю
user_test_pool: Dict[int, List[str]] = {}  # пул вопросов для тестов без повторов
...
def training_nav_keyboard() -> types.ReplyKeyboardMarkup:
    ...
    kb.row(types.KeyboardButton("🏠 Выйти в меню"))
    return kb
...
def get_or_reset_test_pool(user_id: int) -> List[str]:
    """
    Пул id-шников препаров для тестов без повторов.
    Когда кончается – пересоздаём заново.
    """
...
def get_training_list_for_user(user_id: int, category_key: str) -> List[str]:
    """
    Возвращает список id препаров для обучения по разделу
    (или по всем, если RANDOM_CATEGORY_KEY).
    Без повторов: просто тасуем список и идём по нему.
    """
    ...
    st["train_remaining"] = ids
    return ids


def pick_next_training_prep(user_id: int) -> Optional[Preparat]:
    st = user_state.get(user_id)
    if not st or st.get("mode") != "train":
        return None

    category_key = st.get("train_category")
    remaining = st.get("train_remaining") or []

    if not remaining:
        # всё показали — формируем новый цикл
...
def send_preparat_training(chat_id: int, prep: Preparat, with_keyboard: bool = False):
    """
    Отправляет препарат (название + все картинки).
    Если with_keyboard=True — показывает кнопки навигации обучения.
    """
...
def send_mcq_question(chat_id: int, user_id: int, only_errors: bool = False):
    ...
    if only_errors:
        ...
        if not pool:
            # всё спросили — начинаем новый круг
            pool = [p.id for p in PREPARATS]
            random.shuffle(pool)
            user_test_pool[user_id] = pool
        prep_id = pool.pop()
...
def cmd_start(message: types.Message):
    bot.send_message(
        ...
        "Режимы:\n"
        "• <b>Обучение</b> – показываю препарат и его название по разделам.\n"
        "• <b>Случайные препараты</b> – режим внутри обучения.\n"
        "• <b>Тест (варианты)</b> – выбираешь правильный ответ из списка.\n"
        "• <b>Тест (ввод)</b> – сам вписываешь ответ.\n"
...
def handle_training_next(message: types.Message):
    ...
    if not st or st.get("mode") != "train":
        bot.send_message(
            message.chat.id,
            "Сначала выбери раздел обучения через кнопку «📚 Обучение».",
            reply_markup=main_keyboard(),
        )
        return
...
def handle_training_back_to_categories(message: types.Message):
    ...
    st = user_state.get(user_id)
    if st:
        for key in ("mode", "train_category", "train_remaining", "train_current_id"):
            st.pop(key, None)
...
def handle_answer(callback: types.CallbackQuery):
    ...
    if not data or data.get("mode") != "mcq":
        bot.answer_callback_query(
            callback.id,
            "Этот вопрос уже устарел. Нажми «Тест (варианты)».",
            show_alert=True,
        )
        return
...
def handle_typing_answer(message: types.Message):
    ...
    # если пользователь не в режиме "ввод" — игнорируем
